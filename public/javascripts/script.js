  ...
  if (supportsVideo) {
    ...
    if (document.addEventListener) {
      ...
      function createListItem(videoData, id) {
        ...
        button.textContent = `${videoData.fileName} - ${videoData.date} ${videoData.fileSize}`;
        button.addEventListener('click', () => {
          // When the button is clicked, play the video
          video.src = `http://localhost:8000/video?id=${id}`;
          video.load();
          video.play();
          ...
          videoInfo.textContent = `Currently playing: ${videoData.fileName} - ${videoData.date} ${videoData.fileSize}`;
        ...
        deketeButton.addEventListener('click', () => {
          ...
          fetch('http://localhost:8000/deleteVideo', options)
            .then((response) => {
              if (response.ok) {
      ...
      function populateVideoList() {
        ...
        fetch('http://localhost:8000/listVideo', options)
